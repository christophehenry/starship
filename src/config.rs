...
impl Style {
    ...
    pub fn resolve(&self, maybe_previous: Option<&Style>, maybe_next: Option<&Style>) -> Self {
        ...
        if let Some(prev_style) = maybe_previous {
            if let Some(ref_color) = self.bg {
                match ref_color {
                    RefColor::PrevFg => {
                        if let Some(_) = prev_style.fg {
                            panic!("Circular reference: segment background references back to a segment itself referencing forward")
                        }
                        result.style.background = prev_style.style.foreground
                    }
                    RefColor::PrevBg => {
                        if let Some(_) = prev_style.bg {
                            panic!("Circular reference: segment background references back to a segment itself referencing forward")
                        }
            ...
            if let Some(ref_color) = self.fg {
                match ref_color {
                    RefColor::PrevFg => {
                        if let Some(_) = prev_style.fg {
                            panic!("Circular reference: segment foreground references back to a segment itself referencing forward")
                        }
        ...
        if let Some(next_style) = maybe_next {
            if let Some(ref_color) = self.fg {
                match ref_color {
                    RefColor::NextFg => {
                        if let Some(_) = next_style.fg {
                            panic!("Circular reference: segment foreground references forward to a segment itself referencing backwards")
                        }
                        result.style.foreground = next_style.style.foreground
                    }
                    RefColor::NextBg => {
                        if let Some(_) = next_style.bg {
                            panic!("Circular reference: segment foreground references forward to a segment itself referencing backwards")
                        }
            ...
            if let Some(ref_color) = self.bg {
                match ref_color {
                    RefColor::NextFg => {
                        if let Some(_) = next_style.fg {
                            panic!("Circular reference: segment background references forward to a segment itself referencing backwards")
                        }
...
mod tests {
    ...
    fn table_get_styles_previous() {
        ...
        assert_eq!(
            both_prevfg.resolve(Some(&prev_style), Some(&next_style)).to_ansi_style(),
            AnsiStyle::new()
                .fg(Color::Red)
                .on(Color::Yellow)
                .bold()
        ...
        let fg_prev_fg = <StyleWrapper>::from_config(&Value::from("fg:prev_fg"))
            ...
            .0;
        assert_eq!(
            fg_prev_fg.resolve(Some(&prev_style), Some(&next_style)).to_ansi_style(),
        ...
        let fg_prev_bg = <StyleWrapper>::from_config(&Value::from("fg:prev_bg"))
            ...
            .0;
        assert_eq!(
            fg_prev_bg.resolve(Some(&prev_style), Some(&next_style)).to_ansi_style(),
        ...
        let bg_prev_fg = <StyleWrapper>::from_config(&Value::from("bg:prev_fg"))
            ...
            .0;
        assert_eq!(
            bg_prev_fg.resolve(Some(&prev_style), Some(&next_style)).to_ansi_style(),
        ...
        let bg_prev_bg = <StyleWrapper>::from_config(&Value::from("bg:prev_bg"))
            ...
            .0;
        assert_eq!(
            bg_prev_bg.resolve(Some(&prev_style), Some(&next_style)).to_ansi_style(),
